...
window.downloadFile = function(format, event) {
    ...
    const filePath = `data/nomination/nova/summer/female/09-nova-autumn-female-nomination.${format}`
    fetch(filePath)
        .then(response => {
            if (format === 'csv') {
        ...
        .then(data => {
            ...
            a.href = url;
            a.download = `新星组秋季赛提名-女性组别.${format}`;
...
window.downloadCurrentTable = function(format, event) {
    ...
    if (format === 'json') {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = '新星组秋季赛提名-女性组别-当前视图.json';
        document.body.appendChild(a);
        a.click();
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = '新星组秋季赛提名-女性组别-当前视图.csv';
        document.body.appendChild(a);
        a.click();
        URL.revokeObjectURL(url);
        document.body.removeChild(a);
        ...
        try {
            ...
            const worksheet = workbook.addWorksheet('新星组秋季赛提名-女性组别');
            worksheet.columns = [
                { header: '排名', width: 5 },
                { header: '日期', width: 15 },
...
function loadAndSortData(columnIndex, isAsc) {
    fetch("data/nomination/nova/autumn/female/09-nova-autumn-female-nomination.csv")
