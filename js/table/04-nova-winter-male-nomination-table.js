...
window.downloadFile = function(format, event) {
    ...
    const filePath = `data/nomination/nova/winter/male/04-nova-winter-male-nomination.${format}`
    fetch(filePath)
        .then(response => {
            if (format === 'csv') {
        ...
        .then(data => {
            ...
            a.href = url;
            a.download = `新星组冬季赛提名-男性组别.${format}`;
...
window.downloadCurrentTable = function(format, event) {
    ...
    if (format === 'json') {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = '新星组冬季赛提名-男性组别-当前视图.json';
        document.body.appendChild(a);
        a.click();
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = '新星组冬季赛提名-男性组别-当前视图.csv';
        document.body.appendChild(a);
        a.click();
        URL.revokeObjectURL(url);
        document.body.removeChild(a);
        ...
        try {
            ...
            const worksheet = workbook.addWorksheet('新星组冬季赛提名-男性组别');
            ...
            workbook.xlsx.writeBuffer().then(buffer => {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = '新星组冬季赛提名-男性组别-当前视图.xlsx';
                document.body.appendChild(a);
                a.click();
                URL.revokeObjectURL(url);
                document.body.removeChild(a);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedColumn && savedDirection) {
        ...
        // 使用默认排序（票数降序）
        document.querySelector('th:nth-child(8)').classList.add('sort-desc');
        loadAndSortData(7, false);
    }
...
function loadAndSortData(columnIndex, isAsc) {
    fetch("data/nomination/nova/winter/male/04-nova-winter-male-nomination.csv")
