    ...
    sortedData.forEach((data, index) => {
        if (data.votes === -1) {
            ...
            tr.innerHTML = `
                ...
                <td>${data.columns[2]}</td>
                <td>${data.columns[3]}</td>
                <td>${data.columns[4] === '藤田咲' ? `<span class="cv-warning"><span class="tooltip-arrow"></span>${data.columns[4]}</span>` : data.columns[4]}</td>
    ...
    if (searchType === 'none') {
        Array.from(rows).forEach(row => {
            ...
            const matchesStatus = statusFilter === 'all' || 
                                ...
                                (statusFilter === 'promoted' && (isAutoPromoted || parseInt(row.cells[7].textContent) >= 29)) ||
                                (statusFilter === 'not_promoted' && !isAutoPromoted && parseInt(row.cells[7].textContent) < 29);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 绑定表头排序事件
    const sortableColumns = [0, 4, 5, 6, 7]; // 可排序的列索引：排名、角色、作品、声优、票数
    ...
    const savedDirection = localStorage.getItem('tableSortDirection');
    if (savedColumn !== null) {
        const th = document.getElementsByTagName('th')[savedColumn];
        if (th) {
            th.classList.add(savedDirection === 'asc' ? 'sort-asc' : 'sort-desc');
        }
    }
        ...
        setTimeout(() => {
            fetch("data/nomination/stellar/female/01-female-nomination.csv")
                ...
                .then(data => {
                    ...
                    // 设置初始排序状态（如果没有保存的排序状态）
                    if (!localStorage.getItem('tableSortColumn')) {
                        const votesColumn = document.getElementsByTagName('th')[7]; 
                        votesColumn.classList.add('sort-desc');
                        localStorage.setItem('tableSortColumn', '7');
                        localStorage.setItem('tableSortDirection', 'desc');
                    }
...
window.downloadCurrentTable = function(format, event) {
    ...
    if (format === 'json') {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = '恒星组提名-女性组别-当前视图.json';
        document.body.appendChild(a);
        a.click();
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = '恒星组提名-女性组别-当前视图.csv';
        document.body.appendChild(a);
        a.click();
        URL.revokeObjectURL(url);
        document.body.removeChild(a);
        ...
        try {
            ...
            const worksheet = workbook.addWorksheet('恒星组提名-女性组别');
            ...
            workbook.xlsx.writeBuffer().then(buffer => {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = '恒星组提名-女性组别-当前视图.xlsx';
                document.body.appendChild(a);
                a.click();
                URL.revokeObjectURL(url);
                document.body.removeChild(a);
