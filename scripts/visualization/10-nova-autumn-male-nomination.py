...
csv_path = os.path.join(root_dir, 'data', 'nomination', 'nova', 'autumn', 'male', '10-nova-autumn-male-nomination.csv')
df = pd.read_csv(csv_path, encoding='utf-8')

df['得票数'] = pd.to_numeric(df['得票数'].replace('-', '0'))

df_voted = df[df['得票数'] > 0].sort_values(
    ['排名'],
    ascending=True
)
...
advance_data = []
eliminate_data = []
for i, (v, is_advance) in enumerate(zip(votes, df_voted['是否晋级'])):
    if is_advance == True:
        advance_data.append(v)
        eliminate_data.append(None)
    else:
        advance_data.append(None)
        eliminate_data.append(v)

votes = votes[::-1]
labels = labels[::-1]
advance_data = advance_data[::-1]
eliminate_data = eliminate_data[::-1]
...
def create_vote_chart():
    try:
        ...
        output_file = os.path.join(output_dir, '10-nova-autumn-male-nomination.html')
